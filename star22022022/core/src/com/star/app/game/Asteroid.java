...
public class Asteroid implements Poolable {
    ...
    public void render(SpriteBatch batch) {
        batch.draw(textureAsteroid, positionAsteroid.x - 128, positionAsteroid.y - 128, 128, 128,  256, 256, scale,scale, angleAsteroid);
    }
    ...
    public boolean takeDamage(int amount){
        ...
        if ( hp <= 0){
            deactivate();
            if(scale > 0.4f)
            gameController.getAsteroidController().setup(position.x, position.y,MathUtils.random(-150,150),MathUtils.random(-150,150), scale - 0.3f);
            gameController.getAsteroidController().setup(position.x, position.y,MathUtils.random(-150,150),MathUtils.random(-150,150), scale - 0.3f);
            gameController.getAsteroidController().setup(position.x, position.y,MathUtils.random(-150,150),MathUtils.random(-150,150), scale - 0.3f);
            return true;
        }else{
